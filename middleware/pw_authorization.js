        ...
        async (req, res, next) => {
            try {
                ...
                if (!req.body.password) {
                    return res.status(400).json({ msg: "Please provide password.", status: false })
                }
                let token = getToken
                ...
                if (user_id) {
                    ...
                    if (existUser && existUser.accessToken === token) {
                        ...
                        if (!checkPassword) {
                            return res.status(400).json({ msg: 'Password is incorrect. Authentication failed', status: false })
                        } else{ req.role = existUser.role }
                    }
                    else {
                        return res.status(400).json({ msg: "User is logged out or token is invalid or user not exist. Authentication failed.", status: false })
                    }
                } else {
                    return res.status(400).json({ msg: "Session is time out or Token is invalid. Authentication failed.", status: false })
                }
                next()
            }
            catch (error) {
