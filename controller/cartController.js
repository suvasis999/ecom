...
module.exports.addToCart = async (req, res, next) => {
  try {
    ...
    if (!userId || !productId) {
      return res
        .status(400)
        .json({ status: false, msg: "user_id or product_id  not found" });
    }
    ...
    if (alReadyExist != null) {
      return res
        ...
        .send({
          status: false,
          msg: "Product already in cart. Please add quantity instead",
        });
    }
    ...
    if (productDetail && productDetail.stock < qty) {
      return res
        .status(400)
        .send({ status: false, msg: "Product out of stock. Please try again" });
    }
      ...
      if (addingToCart != null) {
        return res
          ...
          .send({
            ...
            data: addingToCart.value,
          });
      } else {
        res
          .status(400)
          .send({ status: false, msg: "Product could not be added to cart" });
      }
    } else {
...
module.exports.wishListToCart = async (req, res, next) => {
  try {
    ...
    if (existWishList == null || existWishList.products.length < 1) {
      return res
        .status(400)
        .send({ status: false, msg: "Wish list not found" });
    }
    await Promise.all(
      existWishList.products.map(async (prod) => {
        const alReadyExist = await Cart.findOne({
  ...
  try {
    ...
    if (!product_id || !user_id || quantity == undefined) {
      return res
        ...
        .send({
          status: false,
          msg: "Please provide product_id & user_id, quantity",
        });
    }
    if (quantity < 1) {
      ...
      if (removeProduct != null) {
        res
          .status(200)
          .send({ status: true, msg: "Product removed from cart" });
      } else {
        res
          .status(200)
          .send({
            status: false,
            msg: "Product can not be remove from cart. Something went wrong",
          });
      }
    }
    ...
    const productInCart = cartDetail.products.find(
      (e) => e.product_id == product_id
    );
    if (!productInCart) {
      return res
        .status(400)
        .send({ status: true, msg: "Product not found in cart" });
    }
    ...
    if (changeQty) {
      res
        ...
        .send({
          status: true,
          data: changeQty,
          msg: "Quantity changed of the product",
        });
    } else {
      res
        .status(200)
        .send({ status: false, msg: "cart details could not be updated" });
    }
  } catch (er) {
    next(er);
  }
...
module.exports.removeProductFromCart = async (req, res, next) => {
  try {
    ...
    if (!product_id || !user_id) {
      return res
        .status(400)
        .send({ status: false, msg: "Please provide product_id & user_id" });
    }
    const removeProduct = await Cart.findOneAndUpdate(
      {
        user_id: user_id,
...
module.exports.ClearCart = async (req, res, next) => {
  try {
    ...
    if (!userId) {
      return res
        .status(200)
        .send({ status: false, msg: "Please provide user_id" });
    }
    await Cart.findOneAndUpdate({user_id:userId},{products:[]})
    res.json({status:true,msg:"Cart cleared."})
  } catch (er) {
    next(er);
  }
};
    ...
    let cartDetails = await Cart.aggregate([
      ...
      { $unwind: "$vendor_details" },
      {
        $addFields: {
          outOfStock: {
            $cond: {
              if: { $lt: ["$product_details.stock", "$products.quantity"] },
              then: true,
              else: false,
            },
          },
      ...
      },
      {
        $group: {
          ...
          grandTotalPrice: {
            $sum: {
              $cond: ["$outOfStock", 0, "$product_details.totalPrice"],
            },
          },
        },
      },
    ...
    if (cartDetails != null) {
      res
        ...
        .send({
          status: true,
          data: cartDetails,
          msg: "User cart details found ",
        });
    } else {
      res
        .status(400)
        .send({ status: false, msg: "User cart details not found " });
    }
  } catch (er) {
    next(er);
  }
